...
import { AlertTriangle, TrendingUp, Zap, Users, ArrowUpRight, CheckCheck, X, Loader2 } from "lucide-react";
import { useT } from "@/lib/i18n";
import { useInsights } from "@/lib/use-insights";
import { cn } from "@/lib/utils";
import type { CompanyInsight } from "@/lib/api/types";

const TYPE_CONFIG: Record<CompanyInsight["type"], { icon: typeof AlertTriangle; color: string; bg: string }> = {
...
export default function InsightsPage() {
  const t = useT();
  const { insights, unreadCount, loading, error, dismiss, dismissAll } = useInsights(50);
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-foreground-muted">
            {unreadCount} {t("insights.unread")} · {insights.length} {t("insights.total")}
          </p>
        </div>
        {unread.length > 0 && (
          <button
          ...
          {unread.length > 0 && (
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {unread.map((ins) => {
                  const { icon: Icon, color, bg } = TYPE_CONFIG[ins.type] || TYPE_CONFIG.risk;
                  return (
                    <div key={ins.insightId} className="card p-6 transition-all duration-200">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {ins.projectId && (
                            ...
                            >
                              ...
                              {ins.relatedTo?.label || ins.projectName || ins.projectId}
                            </Link>
                          )}
                        </div>
                      </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {read.map((ins) => {
                  const { icon: Icon, color, bg } = TYPE_CONFIG[ins.type] || TYPE_CONFIG.risk;
                  return (
                    <div key={ins.insightId} className="card p-5 opacity-75">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-foreground-subtle">
                            {ins.relatedTo ? `${ins.relatedTo.label} · ` : ""}
