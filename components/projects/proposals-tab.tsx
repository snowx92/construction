...
import {
  ...
  RefreshCw, FileText, ShieldCheck, Calendar,
} from "lucide-react";
...
import {
  approveProposal, generateComplianceChecklist, generateProposal, generateSchedule,
  lockProposal, regenerateSection,
...
import type { ProposalSection, ProposalSectionKey, ProposalStatus } from "@/lib/api/types";
...
function ProposalDetail({
  projectId, proposalId, onBack,
}: { projectId: string; proposalId: string; onBack: () => void }) {
  const t = useT();
  const { dir } = useLocale();
  const { profile } = useAuth();
  const companyId = profile?.activeCompanyId;
...
function SectionCard({
  ...
  const { profile } = useAuth();
  const companyId = profile?.activeCompanyId;
  const canRegenerate = useIsOneOf("tender_manager", "estimator", "admin", "company_owner");
  const [regenerating, setRegenerating] = useState(false);

  const key = section.sectionKey;
  const titleKey = key && `proposalsTab.section_${key}`;
  const title = section.title || (titleKey ? t(titleKey) : section.sectionId);
  ...
  async function handleRegen() {
    ...
    try {
      await regenerateSection(proposalId, {
        companyId,
        projectId,
      ...
      <div className="mb-2 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${statusBadge}`}>
            {t(`proposalsTab.sectionStatus_${status}`)}
          </span>
          {section.wordCount && (
